...
class Coolpay_Payment_Helper_Data extends Mage_Core_Helper_Abstract {
    protected $apiUrl     = "https://api.coolpay.com";
    ...
    public function qpCreatePayment(Mage_Sales_Model_Order $order) {
        ...
        $table = $resource->getTableName('coolpaypayment_order_status');
        ...
        if ($textOnStatement = Mage::getStoreConfig('payment/coolpaypayment_payment/text_on_statement')) {
        ...
        $this->apiKey = Mage::getStoreConfig('payment/coolpaypayment_payment/apikey', $storeId);
    ...
    public function capture($payment, $amount) {
        Mage::log('start capture', null, 'coolpay_capture.log');
        ...
        Mage::log($qpOrderStatus, null, 'coolpay_capture.log');
        $coolPayTransactionId = $qpOrderStatus['transaction'];
        ...
        if ((int)($amount * 100) <= ((int)$qpOrderStatus['amount'] - (int)$capturedAmount)) {
            ...
            try {
                $result = $this->qpCapture($coolPayTransactionId, round($amount * 100));
            } catch (Exception $e) {
            ...
            Mage::log($result, null, 'coolpay_capture.log');

            if ($result->qp_status_code == "20000" && ($result->aq_status_code == "000" || $result->aq_status_code == "20000")) {
                $session->addSuccess(Mage::helper('coolpaypayment')->__('Betalingen er hævet online.'));
                ...
                $this->createTransaction($order, $coolPayTransactionId, Mage_Sales_Model_Order_Payment_Transaction::TYPE_CAPTURE);
            } else {
                Mage::throwException("Coolpay Response: " . $result->qp_status_msg);
            }
        } else {
            Mage::throwException(Mage::helper('coolpaypayment')->__('Der forsøges at hæve et højere beløb en tilladt'));
        }

        Mage::log('stop capture', null, 'coolpay_capture.log');
    }
    ...
    public function refund(Mage_Sales_Model_Order $order, $refundtotal) {
        ...
        if (($refundtotal * 100) <= $qpOrderStatus['capturedAmount']) {
            ...
            if ($result->qp_status_code == "20000") {
                $session->addSuccess(Mage::helper('coolpaypayment')->__('Kreditnota refunderet online'));
            ...
            Mage::throwException(Mage::helper('coolpaypayment')->__('Max beløb der kan refunderes: %s', $qpOrderStatus['capturedAmount']));
        }

        $order->addStatusToHistory($order->getStatus(), Mage::helper('coolpaypayment')->__('Kreditnota refunderede %s online', number_format($refundtotal, 2, ",", "")), false);
        $order->save();
    }
    ...
    public function cancel(Mage_Sales_Model_Order $order) {
        ...
        $this->apiKey = Mage::getStoreConfig('payment/coolpaypayment_payment/apikey', $order->getStoreId());
        ...
        if ($result->qp_status_code == "20000") {
            $session->addSuccess(Mage::helper('coolpaypayment')->__('Betalingen blev annulleret online'));
            $write = Mage::getSingleton('core/resource')->getConnection('core_write');
        ...
        $order->addStatusToHistory($order->getStatus(), Mage::helper('coolpaypayment')->__('Betalingen blev annulleret online'), false);
        $order->save();
    }
    ...
    /**
     * Get row from coolpay_order_status table
     *
     * @param $order_id
     *
     * @return bool
     */
    public function getCoolPay($order_id) {
        if ($order_id) {
    ...
    public function getImage($order_id) {
        ...
        if ($order_id) {
            $order = Mage::getModel('sales/order')->load($order_id);
            if ($order) {
                $payment = $this->getCoolPay($order_id);
    ...
    public function getInstalledVersion() {
        return (string)Mage::getConfig()->getNode()->modules->Coolpay_Payment->version;
    }
}
