...
class Coolpay_Payment_PaymentController extends Mage_Core_Controller_Front_Action {
    /**
     * Get payment method
     *
     * @return Coolpay_Payment_Model_Payment
     */
    public function getPayment() {
        return Mage::getSingleton('coolpaypayment/payment');
    }

    /**
     * Handle redirect to CoolPay
     */
    public function redirectAction() {
        ...
        $session->setCoolpayQuoteId($session->getQuoteId());

        $block = Mage::getSingleton('core/layout')->createBlock('coolpaypayment/payment_redirect');
        $block->toHtml();
    ...
    public function cancelAction() {
        ...
        $session = $this->_getSession();
        $session->setQuoteId($session->getCoolpayQuoteId(true));
        if ($session->getLastRealOrderId()) {
            ...
            Mage::helper('coolpaypayment/checkout')->restoreQuote();
        }
    ...
    /**
     * Handle customer being redirected from CoolPay
     */
    ...
    public function callbackAction() {
        Mage::log("Logging callback data", null, 'coolpay_callback.log');

        $requestBody = $this->getRequest()->getRawBody();
        $request = json_decode($requestBody);

        Mage::log($request, null, 'coolpay_callback.log');
        ...
        if ($checksum == $_SERVER["HTTP_QUICKPAY_CHECKSUM_SHA256"]) {
            ...
            // Save the order into the coolpaypayment_order_status table
            ...
            if ($request->accepted && $operation->type == 'authorize') {
                if ($operation->pending == true) {
                    Mage::log('Transaction accepted but pending', null, 'coolpay_callback.log');
                } else {
                    Mage::log('Transaction accepted', null, 'coolpay_callback.log');
                }
                if ((int)$payment->getConfigData('transactionfee') == 1) {
                    $fee = $operation->amount - ($order->getGrandTotal() * 100.0);
                    $fee = ((int)$fee / 100.0);
                    Mage::log('Transaction fee added: ' . $fee, null, 'coolpay_callback.log');
                    $fee_text = "";
                    if ((int)$payment->getConfigData('specifytransactionfee') == 1) {
                        $fee_text = " " . Mage::helper('coolpaypayment')->__("inkl. %s %s i transaktionsgebyr", $fee, $order->getData('order_currency_code'));
                    }
                ...
                Mage::log($query, null, 'coolpay_callback.log');
                ...
                Mage::helper('coolpaypayment')->createTransaction($order, $request->id, Mage_Sales_Model_Order_Payment_Transaction::TYPE_AUTH);

                /*
                 * If test mode is disabled and the order is placed with a test card, reject it
                 * We wait until this moment since qpCancel will fail without a row in coolpaypayment_order_status
                 */
                if (!$payment->getConfigData('testmode') && $request->test_mode == 1) {
                    Mage::log('Attempted callback with test card while testmode is disabled for order #' . $order->getIncrementId(), null, 'coolpay_debug.log');
                    //Cancel order
                    if ($order->canCancel()) {
                        try {
                            $order->cancel();
                            ...
                            Mage::log('Failed to cancel testmode order #' . $order->getIncrementId(), null, 'coolpay_debug.log');
                        }
                    }
                ...
                if ((int)Mage::getStoreConfig('cataloginventory/item_options/manage_stock') == 1) {
                    Mage::helper('coolpaypayment')->removeFromStock($order->getIncrementId());
                }
            }
        } else {
        ...
        // Callback from Coolpay - just respond ok
