...
abstract class Coolpay_Payment_Model_Method_Abstract extends Mage_Payment_Model_Method_Abstract {
    protected $_formBlockType = 'coolpaypayment/payment_form';
    protected $_infoBlockType = 'coolpaypayment/info_coolpay';
    ...
    public function getOrderPlaceRedirectUrl() {
        return Mage::getUrl('coolpaypayment/payment/redirect');
    }
    ...
    public function canUseInternal() {
        ...
        if (Mage::app()->getFrontController()->getAction() instanceof Mage_Adminhtml_Sales_Order_EditController) {
            // Her kunne testes om det i forvejen er en Coolpay order.
    ...
    /**
     * Validate if the currency code is avaialable to use for Coolpay or not
     *
     * @return $this
     */
    public function validate() {
        ...
        if (!in_array($currency_code, $this->_allowCurrencyCode)) {
            Mage::throwException(Mage::helper('coolpaypayment')->__('Valutakoden (%s) er ikke kompatible med Coolpay', $currency_code));
        }
    ...
    public function calcLanguage($lan) {
        ...
        $lang = $splitted[0];
        if (isset ($map_codes[$lang])) {
            return $map_codes[$lang];
        }
        return $lang;
    }
    ...
    public function canCapturePartial() {
        ...
        if ($controller instanceof Mage_Adminhtml_Sales_Order_CreditmemoController) { // allow editing of qty in creditmemo
            return true;
        } elseif ($controller instanceof Mage_Adminhtml_Sales_Order_InvoiceController) { // allow editing of qty in invoice
            return true;
        }
     ...
     * @param float         $amount
     *
     * @return Coolpay_Payment_Model_Method_Abstract
     */
    public function capture(Varien_Object $payment, $amount) {
        Mage::log('start capture', null, 'coolpay_capture.log');

        try {
            Mage::helper('coolpaypayment')->capture($payment, $amount);
        } catch (Exception $e) {
            Mage::throwException(Mage::helper('coolpaypayment')->__('Ikke muligt at hæve betalingen online, grundet denne fejl: %s', $e->getMessage()));
        }
    ...
    public function refund(Varien_Object $payment, $amount) {
        try {
            ...
            Mage::helper('coolpaypayment')->refund($order, $amount);
        } catch (Exception $e) {
            Mage::getSingleton('adminhtml/session')->addException($e, Mage::helper('coolpaypayment')->__('Ikke muligt at refundere betalingen online, grundet denne fejl: %s', $e->getMessage()));
        }
    ...
    public function void(Varien_Object $payment) {
        try {
            $order = $payment->getOrder();
            Mage::helper('coolpaypayment')->cancel($order);
        } catch (Exception $e) {
            Mage::getSingleton('adminhtml/session')->addException($e, Mage::helper('coolpaypayment')->__('Ikke muligt at annullerer betalingen online, grundet denne fejl: %s', $e->getMessage()));
        }
    ...
    public function getTitle() {
        ...
        if (Mage::app()->getFrontController()->getAction() instanceof Mage_Adminhtml_Sales_Order_EditController) {
            ...
            return $this->getConfigData('title') . " - " . Mage::helper('coolpaypayment')->__('Maks beløb:') . " " . $row['amount'] / 100 . " " . $row['currency'];
        }
    ...
    public function getConfigData($field, $storeId = null) {
        ...
        if (Mage::getStoreConfig($path, $storeId) === null) {
            $path = 'payment/coolpaypayment_payment/' . $field;
        }
