...
class Coolpay_Payment_Model_Observer {
    public function autoRegisterState(Varien_Event_Observer $observer) {
        ...
        if (isset($data['coolpay_state'])) {
            Mage::getSingleton('core/session')->setCoolpayState($data['coolpay_state']); // Branding
        }
    ...
    */
    public function placeOrder($observer) {
        try {
            ...
            if ($payment instanceof Coolpay_Payment_Model_Method_Abstract) {
    ...
    public function saveOrder($observer) {
        ...
        try {
            ...
            $session->addException($e, Mage::helper('coolpaypayment')->__("Can't change status of order", $e->getMessage()));
        }
    ...
    public function onBlockHtmlBefore(Varien_Event_Observer $observer) {
        ...
        if ($block->getType() === 'adminhtml/sales_order_grid') {
            ...
            $block->addColumnAfter('fraudprobability', array(
                ...
                'width'    => '40px',
                'weight'   => '100',
                'renderer' => 'Coolpay_Payment_Model_Sales_Order_Grid_Fraudprobability',
            ), 'status');
    ...
    public function onCheckoutSubmitAllAfter(Varien_Event_Observer $observer) {
        ...
        if ($order->getPayment()->getMethodInstance() instanceof Coolpay_Payment_Model_Method_Abstract) {
            $payment = Mage::getSingleton('coolpaypayment/payment');
    ...
    public function getSuccessUrl(Mage_Core_Model_Store $store) {
        return $store->getUrl('coolpaypayment/payment/linksuccess');
    }
    ...
    public function getCancelUrl(Mage_Core_Model_Store $store) {
        return $store->getUrl('coolpaypayment/payment/cancel');
    }
    ...
    public function getCallbackUrl(Mage_Core_Model_Store $store) {
        return $store->getUrl('coolpaypayment/payment/callback');
    }
}
